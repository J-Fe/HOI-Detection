...
###
class TSNE_VisualModelV(nn.Module):
    def __init__(self,args,out_type = None):
        super(TSNE_VisualModelV, self).__init__()
        ...
        self.appearence_spatial_subnet = GAT2(self.appearence_in_dim,-1,self.out_dim,feat_drop=self.feat_drop,attn_drop=self.attn_drop,activation=nn.ReLU())
        ...
        self.spatial_temporal_subnet = GAT2(self.appearence_in_dim,-1,self.out_dim,feat_drop=self.feat_drop,attn_drop=self.attn_drop,activation=nn.ReLU())
    ...
    def forward(self,num_objs, node_features, box_input, box_categories,out_type='scores'): # appearance_feats,box_tensors,box_categories
        ...
        self.classifier_tsne = nn.Sequential(
            nn.Linear(4*self.out_dim, 2*self.out_dim),
            # nn.BatchNorm1d(self.embedding_feature_dim),
            nn.Dropout(self.cls_dropout),
            nn.ReLU(inplace=True),
        ...
        TSNE_features = torch.sum(self.classifier_tsne(human_rnn_feats), dim=1)
        ...
        #Sub_TSNE = torch.sum(self.classifier_tsne(human_rnn_feats), dim=1)
       # Aff_TSNE = torch.sum(self.classifier_tsne(obj_rnn_feats), dim=1)
        #print(Sub_TSNE.shape)
        #ff
        
        return subact_cls_scores, afford_cls_scores , TSNE_features
...
class SemanticModelV(nn.Module):
    def __init__(self,args,out_type = None):
        ...
        self.semantic_spatial_subnet = GAT2(self.semantic_in_dim,-1,self.out_dim,feat_drop=self.feat_drop,attn_drop=self.attn_drop,activation=nn.ReLU())
        ...
        self.spatial_temporal_subnet = GAT2(self.semantic_in_dim,-1,self.out_dim,feat_drop=self.feat_drop,attn_drop=self.attn_drop,activation=nn.ReLU())
